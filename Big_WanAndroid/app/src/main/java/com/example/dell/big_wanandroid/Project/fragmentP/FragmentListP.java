...
public class FragmentListP extends BaseFragment<ProjectListP, ProjectListView> implements ProjectListView {
    ...
    @BindView(R.id.rlv)
    RecyclerView rlv;
    @BindView(R.id.fllbtn)
    FloatingActionButton fllbtn;
    private int page = 1;
    private int cid;
    ...
    public void SetListData(ListBeanP listBeanP) {
        ...
        fllbtn.setVisibility(View.GONE);

        RlvHideUtils.TabHide(rgbtn, rlv, flbtn);
        RlvHideUtils.OnTabClicks(flbtn, rlv, Toolbar, tab);
        RlvHideUtils.initTabRecy(rlv, Toolbar,flbtn);
    }
